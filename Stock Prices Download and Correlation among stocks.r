...
# Intialize your tickers
ticker <- c("JPM", "WFC", "BAC", "C", "GS", "V", "AIG", "MS", "CME", "DFS")

# tabe for stroring log returns
daily.log.return <- NULL

#Download data and store log returns in the table
for(i in 1:10){
...
# Calculate monthly reyturns for same tickers
for(i in 1:10){
  table1 <- getSymbols(ticker[i], from = "2014-01-01", auto.assign = F)
...
# Normality test for each equity
...
# equity having higest correlation
which.max(cor.table)
...
# Graphs for 2 highest correlated stocks
